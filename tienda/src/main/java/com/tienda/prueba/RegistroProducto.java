...
public class RegistroProducto {
	 ...
	 *  			Modelo entidad = em.find(modelo.class, codigoPK) <- recupera una entidad especifica de la base de datos
	...
	public static void main(String[] args) {
		...
		Productos producto = pd.buscarPorID(1);
		System.out.println(producto.getNombre());
		
		List<Productos> productos = pd.buscarTodos();
		productos.forEach(produ -> System.out.println(produ.getDescripcion()));
	}
	...
	private static void guardarProducto() {
		...
		ProductosDao productoDao = new ProductosDao(em);
		CategoriasDao categoriaDao = new CategoriasDao(em);
